...
var min_window = 150;   // Do not display less than this many bases - modified in runtime, value depends on image width which can be changed
...
function get_viewport_width ( elm ) {
	if (typeof elm.innerWidth != 'undefined') {
...
function update_image () {
	...
	cur_to = Math.floor ( document.getElementById('chr_to').value ) ;
	
	if ( cur_to > maxw ) {
		cur_to = maxw ;
		document.getElementById('chr_to').value = maxw ;
	}
	
	if ( cur_to - cur_from > 25000 ) {
		if ( display_mode == 'pileup' || display_mode == 'paired_pileup' ) {
	...
	document.getElementById("button_zoom_150").disabled = cur_to - cur_from + 1 <= 152 ;
	document.getElementById("button_zoom_in").disabled = cur_to - cur_from + 1 <= 152 ;
	...
	document.getElementById('button_right').disabled = cur_to == maxw ;
	document.getElementById('button_zoom_out').disabled = cur_from == 1 && cur_to == maxw ;
	document.getElementById('button_zoom_chr').disabled = cur_from == 1 && cur_to == maxw ;
	...
	display += document.getElementById('show_faceaway').checked ? 'faceaway|' : '' ;
...
//	document.getElementById('img_container').style.position = '' ;
	...
	if ( new_range < 100 ) {
		if ( window.single_position_double_click ) single_position_double_click ( pos_x , pos_y ) ;
		return ;
	}
...
function show_new_range ( new_middle , new_range ) {
	...
	if ( new_range < 100 ) return ; // No need to zoom closer
...
function zoom_in () {
	var new_middle = Math.floor ( cur_from + ( cur_to - cur_from ) / 2 ) ;
	var new_range = Math.floor ( ( cur_to - cur_from + 1 ) / 2 ) ;
	if ( new_range < 150 ) new_range = 150 ;
	var new_from = Math.floor ( new_middle - new_range / 2 ) ;
	var new_to = new_from + new_range ;
	while ( new_from < 1 ) {
		new_from++ ;
...
function mouse_moved_over_main_image ( event ) {
    ...
    var pos_x = event.offsetX?(event.offsetX):event.pageX - document.getElementById("main_image").offsetLeft - getElementPosition('main_image').left;
    var pos_y = event.offsetY?(event.offsetY):event.pageY - document.getElementById("main_image").offsetTop;
	
//	pos_x = event.pageX - getElementPosition('main_image').left ;
...
//	pos_x -= getElementPosition('main_image').left ;
	...
	var off_x = 0 ;//getElementPosition('main_image').left ;
	var vruler = document.getElementById('vruler') ;
	vruler.style.height = h + "px" ;
	vruler.style.left = ( pos_x - 1 ) + "px" ;
...
function full_chromosome_rezise () {
	...
	document.getElementById('chr_to').value = chromosome_length[get_selected_chromosome()] ;
}
...
// Event handler : Handles faceaway display changes.
function show_faceaway_changed () {
    update_image();
}
...
function image_width_changed () {
	change_image_width ( document.getElementById('image_width').value ) ;
}

function change_image_width ( nw ) {
...
function on_resize () {
	var nw = get_viewport_width ( document.getElementById('img_container') ) ;
...
//	three_d_ize ( document.getElementById('img_container') ) ;
	...
	document.getElementById('show_faceaway').checked			= b['faceaway'] ? true : false ;
...
function update_hbar () {
	...
	dragbar.style.left = Math.floor ( img_width * cur_from / maxw ) + "px" ;
	dragbar.style.width = Math.floor ( img_width * curw / maxw ) + "px" ;
}
